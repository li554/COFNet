...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        target_masks = box2masks(cu_targets, img_rgb, device)
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, _, train_out = model(img_rgb, img_ir, target_masks, augment=augment, p=opt.res_mode)  # inference and training outputs
            out = out[:3]
        ...
        for si, pred in enumerate(out):
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    # line = (int(cls), *xywh, conf) if save_conf else (int(cls), *xywh)  # label format
    ...
    if "KAIST" in data['path'] or "kaist" in data['path']:
        # 计算MR指标
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='weights/best_flir.pt',
                        help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/multispectral/FLIR-align-3class.yaml', help='*.data path')
    parser.add_argument('--res_mode', type=str, default="predicted", choices=["real", "random", 'predicted'], help='pooling mode')
