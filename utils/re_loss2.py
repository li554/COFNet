...
from utils.general import bbox_iou, box2mask
from utils.torch_utils import is_parallel
from utils.mask_loss import _neg_loss
...
class MaskLoss(nn.Module):
    def __init__(self):
        ...
        self.focal_loss = _neg_loss
        # self.focal_loss = FocalLoss(nn.BCEWithLogitsLoss())

    def forward(self, layer_mask_conv, layer_mask_f, loss_type):
        mask_loss = .0
        # R = torch.tensor([mask_conv.shape[2] for mask_conv in layer_mask_conv],
        #                  dtype=torch.float32, device=layer_mask_conv[0].device)
        # R = 1 / R
        # weights_layer = R / R.sum()
        weights_layer = [1] * len(layer_mask_conv)
        for i in range(len(layer_mask_conv)):
            mask_conv = layer_mask_conv[i]
            mask_f = layer_mask_f[i]
            if loss_type == "mse":
                mask_loss += F.mse_loss(torch.sigmoid(mask_conv), mask_f) * weights_layer[i]
            elif loss_type == "smooth_l1":
                mask_loss += F.smooth_l1_loss(torch.sigmoid(mask_conv), mask_f) * weights_layer[i]
            elif loss_type == "cos":
                mask_loss += F.cosine_embedding_loss(torch.sigmoid(mask_conv), mask_f,
                                                     torch.ones(mask_conv.size(0)).cuda()) * weights_layer[i]
            elif loss_type == "focal":
                mask_loss += self.focal_loss(mask_conv, mask_f) * weights_layer[i]
            elif loss_type == "bce":
                mask_loss += F.binary_cross_entropy_with_logits(mask_conv, mask_f) * weights_layer[i]
        if len(layer_mask_conv) != 0:
            mask_loss /= len(layer_mask_conv)
        return mask_loss
...
class ContrastLoss(nn.Module):
    ...
    def forward(self, feature_pool=None, mask_pool=None):
        mask_f = F.normalize(mask_pool, p=2, dim=1)
        labels = torch.arange(len(mask_pool), device=mask_pool.device)  # 对角线元素的labels
        ...
        sim_loss = (vi_loss_i + vi_loss_t) / 2
        return sim_loss
...
class RankingLoss2(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        for x, y in zip(pred, true):
            ...
            loss -= y * torch.log(1 - (y - x) + 1e-7) + (1 - y) * torch.log(
                torch.where(self.threshold - (1 - x) > 0, 1 - self.threshold + (1 - x) + 1e-7,
                            torch.ones([1], dtype=x.dtype, device='cuda')))
...
class ComputeLoss:
    ...
    def __call__(self, p, targets, imgsz=(640, 640)):  # predictions, targets, model
        self.nl = len(p)
        device = targets.device
        lcls, lbox, lobj, lrk, re_loss = (torch.zeros(1, device=device), torch.zeros(1, device=device),
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # # Restruction
                # tdata = torch.cat((tbox[i], tcls[i].unsqueeze(1).float()), 1)
                # pdata = torch.cat((pbox, ps[:, 5:].argmax(dim=1).unsqueeze(1).float()), 1)
                # pmask = box2mask(pdata, imgsz, device)
                # tmask = box2mask(tdata, imgsz, device)
            ...
            lobj += obji * self.balance[j]  # obj loss
            if self.autobalance:
                self.balance[j] = self.balance[j] * 0.9999 + 0.0001 / obji.detach().item()
        ...
        return loss * bs, re_loss * bs, torch.cat((lbox, lobj, lcls, lrk)).detach()
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            # if self.nl>na and self.nl == 4 and i == 3:
            #     j = 2
            # else:
            #     j = i % 3
            j = i
            anchors, shape = self.anchors[j], p[i].shape
...
if __name__ == '__main__':
    # test focal loss
    focal_loss = FocalLoss(nn.BCELoss(), gamma=1.5, alpha=0.25)
    # 随机生成一些预测和真实标签作为示例
    pred = torch.rand((1, 1, 128, 128))  # 假设为 128x128 的概率图像
    gt = torch.randint(0, 2, (1, 1, 128, 128)).float()  # 128x128 的真实掩码图像

    output = focal_loss(pred, gt)
    print(output)
