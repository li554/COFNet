...
class MaskLoss(nn.Module):
    ...
    def forward(self, layer_mask_conv, layer_mask_f):
        mask_loss = .0
        for i in range(len(layer_mask_conv)):
            mask_conv = layer_mask_conv[i]
            mask_f = layer_mask_f[i]
            # mask_loss += F.mse_loss(mask_conv, mask_f)
            mask_loss += F.smooth_l1_loss(mask_conv, mask_f)
            # mask_loss += F.cosine_embedding_loss(mask_conv, mask_f, torch.ones(mask_conv.size(0)).cuda())
            # mask_loss += self.focal_loss(mask_conv, mask_f)
            # mask_loss += F.binary_cross_entropy_with_logits(mask_conv, mask_f)
