...
class Model(nn.Module):
    ...
    def forward(self, x, x2=None, x3=None, x4=None, augment=False, profile=False):
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                yi = self.forward_once(xi, x2, x3, x4)[0]  # forward
                yi[..., :4] /= si
                if fi == 2:
            ...
            return self.forward_once(x, x2, x3, x4, profile)  # single-scale inference, train

    def forward_once(self, x, x2=None, x3=None, x4=None, profile=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,
                        default='transformer/yolov5l_MaskFusion_mask_FLIR_wb.yaml',
                        help='model.yaml')
    ...
    model = Model(opt.cfg, ch=3).to(device)
    input_rgb = torch.randn(4, 3, 640, 640).to(device)
    input_ir = torch.randn(4, 3, 640, 640).to(device)
    mask = torch.randn(4, 3, 640, 640).to(device)
    ...
    with torch.no_grad():
        ...
        gflops /= 2
        print('gflops:', gflops / 1E9, 'params:', params / 1E6)
