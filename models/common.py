...
from models.conv_transformer import DeepWiseConv, NewTransformerBlock
...
class CrossAttention2(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        # output
        out_vis = torch.matmul(att, v_vis).permute(0, 2, 1, 3).contiguous().view(b_s, nq,
                                                                                 self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_vis = self.resid_drop(self.out_proj_vis(out_vis))  # (b_s, nq, d_model)
        out_ir = torch.matmul(att, v_ir).permute(0, 2, 1, 3).contiguous().view(b_s, nq,
                                                                               self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_ir = self.resid_drop(self.out_proj_ir(out_ir))  # (b_s, nq, d_model)
...
class CrossAttention3(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        k_fuse = self.key_proj(fuse_fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 3,
                                                                                                   1)  # (b_s, h, d_k, nk) K^T
        v_fuse = self.val_proj(fuse_fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 1,
                                                                                                   3)  # (b_s, h, nk, d_v)
        ...
        # output
        out_vis = torch.matmul(att_vis, v_fuse).permute(0, 2, 1, 3).contiguous().view(b_s, nq,
                                                                                      self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_vis = self.resid_drop(self.out_proj_vis(out_vis))  # (b_s, nq, d_model)
        out_ir = torch.matmul(att_ir, v_fuse).permute(0, 2, 1, 3).contiguous().view(b_s, nq,
                                                                                    self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_ir = self.resid_drop(self.out_proj_ir(out_ir))  # (b_s, nq, d_model)
...
class SEFusionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ir_fea = x[1]
        assert rgb_fea.shape[0] == ir_fea.shape[0]
        x = self.reduce_conv(torch.cat([rgb_fea, ir_fea], dim=1))
        return x * self.att(x)
...
class AttentionPooling(nn.Module):
    def __init__(self, in_channels, out_channels, pool_size=(4, 4)):
        ...
        self.spatial_att = nn.Conv2d(in_channels, 1, kernel_size=1, bias=False)
        self.channel_att = nn.Conv2d(in_channels, in_channels, kernel_size=1, groups=in_channels, bias=False)
    ...
    def forward(self, x):
        ...
        spatial_att = self.spatial_att(x)  # (batch_size, 1, height, width)
        spatial_att = torch.softmax(spatial_att.view(spatial_att.size(0), -1), dim=-1)  # (batch_size, height*width)
        spatial_att = spatial_att.view(spatial_att.size(0), 1, x.size(2), x.size(3))  # (batch_size, 1, height, width)

        x = x * spatial_att  # (batch_size, channels, height, width)
        channel_att = self.channel_att(x)  # (batch_size, channels, height, width)
        channel_att = torch.softmax(channel_att.view(channel_att.size(0), channel_att.size(1), -1), dim=-1).sum(
...
class ConvPooling(nn.Module):
    def __init__(self, in_c=3, out_c=3, pool_size=(4, 4), kernel_size=3, stride=1, padding=1, bias=False):
        super(ConvPooling, self).__init__()
        self.proj = DeepWiseConv(in_c, out_c, kernel_size=kernel_size, stride=stride, padding=padding, bias=bias)
        self.pooling = nn.AdaptiveAvgPool2d(pool_size)
...
class SpatialAttention(nn.Module):
    def __init__(self):
        super().__init__()
        self.spatial_attention = nn.Sequential(
            nn.Conv2d(in_channels=2, out_channels=1, kernel_size=7, stride=1, dilation=1,
                      padding=(7 - 1) // 2, bias=False),
            nn.BatchNorm2d(num_features=1, eps=1e-5, momentum=0.01, affine=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        x = x * self.spatial_attention(x_compress)
        return x
...
if __name__ == '__main__':
    in_channels = 256
    feature_size = 20
    model = MaskGuideFusionBlock(in_channels)
    model.train()

    input = [torch.randn(1, in_channels, feature_size, feature_size),
             torch.randn(1, in_channels, feature_size, feature_size),
             torch.randn(1, in_channels, feature_size, feature_size)]

    mask = torch.zeros(1, 3, 640, 640)
    mask[0, 0, 100:200, 100:200] = 1
    output = model(input, p="sam", x3=mask)
