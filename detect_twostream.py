...
from utils.general import check_requirements, check_imshow, non_max_suppression, apply_classifier, \
...
def detect(opt):
    ...
    for k, v in state_dict.items():
        l = int(k.split('.')[1])
        if k == 'model.43.anchors' or k == 'model.43.anchor_grid':
            v = v[:-1]
        elif 20 <= l <= 25 or 'model.43.m.3' in k:
            continue
        values.append(v)
    model_state_dict = model.state_dict()
    for i, k in enumerate(model_state_dict.keys()):
        model_state_dict[k] = values[i]
    ...
    for (path, img, im0s, vid_cap), (path_, img2, im0s_, vid_cap_) in zip(dataset, dataset2):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # #
            # # ---------------------------------------------------------------------------
            # annoPath = "data/visualization/labels/"
            # # annoName  = (path_.split("/")[-1]).split(".")[0] + ".txt"
            # annoName = p.name.split(".")[0] + ".txt"
            # annoPath += annoName
            # # print(annoPath)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/sota2/LLVIP_avg_pool2/weights/best.pt',
                        help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='./models2/transformer/yolov5l_MaskFusion_mask_LLVIP_wb.yaml',
                        help='model.yaml')
    parser.add_argument('--source1', type=str, default='D:/PycharmProjects/Detection/multispectral/ICAFusion/data/LLVIP/visible/train/010042.jpg',
                        help='source')  # file/folder, 0 for webcam
    parser.add_argument('--source2', type=str, default='D:/PycharmProjects/Detection/multispectral/ICAFusion/data/LLVIP/infrared/train/010042.jpg',
                        help='source')  # file/folder, 0 for webcam
    ...
    parser.add_argument('--name', default='Ours2', help='save results to project/name')
