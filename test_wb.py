...
from models.yolo_mask_infer import Model
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, nc=len(ckpt['model'].names))
        ...
        # print model flops and params
        input = torch.randn(1, 3, imgsz, imgsz).to(device)
        flops, params = thop.profile(model, inputs=(input, input, input))
        print(f'FLOPs: {flops / 1E9:.4g} G')
        print(f'Params: {params / 1E6:.4g} M')
    ...
    if nc == 1:
        s = ('%20s' + '%12s' * 10) % (
            'Class', 'Images', 'Labels', 'TP', 'FP', 'FN', 'F1', 'P', 'R', 'mAP@.5', 'mAP@.5:.95')  # 设置进度条的显示信息
    else:
    ...
    # print("learning temperature: ", model.learned_temperature)
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        img = img.to(device, non_blocking=True)
        img = img.half() if half else img.float()  # uint8 to fp16/32
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
        targets = targets.to(device)
        nb, cc, height, width = img.shape  # batch size, channels, height, width

        img_rgb = img[:, :3, :, :]
        img_ir = img[:, 3:, :, :]

        target_masks = torch.zeros((nb, cc // 2, height, width), device=device)
        target_masks = target_masks.half() if half else target_masks.float()

        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, _, train_out = model(img_rgb, img_ir, target_masks, augment=augment)  # inference and training outputs
            out = out[:3]
            out = torch.cat(out, dim=1)
            train_out = train_out[:3]

            t0 += time_synchronized() - t

            # Compute loss
            if compute_loss:
                loss += compute_loss([x.float() for x in train_out], targets, (height, width))[2][:4]  # box, obj, cls
    ...
    if not isinstance(tp, int):
        return (
            tp[0], fp[0], fn[0], f1[0], mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, MRresult, t
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='weights/COFNet_FLIR.pt',
                        help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov5l_MaskFusion_mask_FLIR_wb.yaml',
                        help='model.yaml')
    parser.add_argument('--data', type=str, default='./data/multispectral/FLIR-align-3class.yaml', help='*.data path')
    parser.add_argument('--batch-size', type=int, default=1, help='size of each image batch')
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')  # 0.05: 818
    ...
    print(opt.data)
    check_requirements()
    root = "D:/PycharmProjects/Detection/multispectral/ICAFusion/data"
    if "kaist" in opt.data:
        p = f"{root}/KAIST/labels/test"
    elif "FLIR" in opt.data:
        p = f"{root}/FLIR-align-3class/labels/test"
    elif "LLVIP" in opt.data:
        p = f"{root}/LLVIP/labels/test"
    elif "VEDAI" in opt.data:
        p = f"{root}/VEDAI/labels/test"
    elif "visualization" in opt.data:
        p = "data/visualization/labels/"
    labels_list = os.listdir(p)
    labels_list.sort()
    if opt.data in ['./data/multispectral/FLIR-align-3class.yaml', './data/multispectral/FLIR-ADAS.yaml',
                    './data/multispectral/VEDAI.yaml']:
        opt.verbose = True
